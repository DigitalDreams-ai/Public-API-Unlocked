...
This package is intentionally configurable through `PublicApi_Integration_Config__c` and does not require a dependency on any specific vertical package. Guest requests are recorded on `PublicApi_Submission__c`, then internal async processing creates the configured target object.

To test the Public API Submissions API in dev via Experience Cloud (guest, no session), see [Experience Cloud site setup for API testing](docs/experience-site-dev-setup.md).
